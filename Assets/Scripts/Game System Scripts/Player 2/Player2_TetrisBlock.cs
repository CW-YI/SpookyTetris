...
public class Player2_TetrisBlock : TetrisBlock
{
    ...
    #region 다른 스크립트 참조
    private Player2_TetrominoSpawner tetrominoSpawner;
    #endregion
    ...
    public delegate void SendGarbageLinesToOpponent(int rows, string player);
    public static event SendGarbageLinesToOpponent OnSendGarbageLinesToOpponent;

    public delegate void P2_GameOver();
    public static event P2_GameOver OnGameOver;
    #endregion
   ...
   // public static float timeElapsedInSeconds = 0f;
    ...
    private void Awake()
    {
        tetrominoSpawner = FindAnyObjectByType<Player2_TetrominoSpawner>();
    }
    void Start()
    {
        if (!hasGrid)
        {
            grid_2 = new Transform[width + 3, height + 1];
            hasGrid = true;
        }
        isLastMoveRotation = false;
        isValidPlacement = false;
        if (PhotonNetwork.IsConnected && !PhotonNetwork.IsMasterClient) InstantiateGhostPiece();
    }
    ...
    void MoveTetrominoes_Player2()
    {
        ...
        if (Time.time - previousTime > (Input.GetKey(KeyCode.DownArrow) ? fallTime / 10 : fallTime)) //아래 방향키 눌러서 빨리 떨구기
        {
            ...
            if (!ValidMove(gameObject))
            {
                ...
                this.enabled = false;
                FindObjectOfType<Player2_TetrominoSpawner>().NewTetromino();
    ...
    bool GroundCheck()
    {
        ...
        if (grid_2[lowestX, lowestY - 1] != null) return true;
        return false;
    }
    ...
    public int GetHighestY()
    {
        ...
        for (int x = leftMostXAxis; x <= width; x++)
        {
            for (int y = height; y >= bottomHeight; y--)
            {
                if (grid_2[x, y] != null)
                {
                    if (!grid_2[x, y].gameObject.CompareTag("Unremoveable"))
                    {
                        if (y > highestY)
                        {
                            highestY = y;
    ...
    bool TSpinCheck()
    {
        ...
        { 
            ...
            {
                if (grid_2[x, y + 1] != null) count++;
                if (grid_2[x, y - 1] != null || y - 1 == 0) count++;
            }
            else if (rotation == 90 || rotation == 270)
            {
                if (grid_2[x + 1, y] != null) count++;
                if (grid_2[x - 1, y] != null || x < leftMostXAxis) count++;
            }
        }
    ...
    public void CheckForLines()
    {
        ...
        if (rowsDeleted > 0)
        {
            OnSendGarbageLinesToOpponent?.Invoke(rowsDeleted, "Player2");
        }
    }
    bool HasLine(int i)
    {
        ...
        {
            if (grid_2[j, i] == null || (grid_2[j, i] != null && grid_2[j, i].gameObject.CompareTag("Unremoveable"))) return false;
        }
    ...
    bool HasGarbageLine(int i)
    {
        for (int j = leftMostXAxis; j <= width; ++j)
        {
            if (grid_2[j, i].gameObject.CompareTag("Garbage")) return true;
        }
    ...
    IEnumerator DeleteLine(int i)
    {
        ...
        for (int j = leftMostXAxis; j <= width; ++j)
        {
            grid_2[j, i].gameObject.layer = 6;
            grid_2[j, i].gameObject.GetComponent<PostProcessVolume>().profile.TryGetSettings(out Bloom bloom);
            bloom.intensity.value = 20f;
        }
    ...
    void deleteLine(int i)
    {
        for (int j = leftMostXAxis; j <= width; ++j)
        {
            if (grid_2[j, i] != null)
            {
                Destroy(grid_2[j, i].gameObject);
                grid_2[j, i] = null;

            }
            //else Debug.Log(grid_2[j, i].name);
        }
        ...
        for (int y = i; y < height; ++y)
        {
            for (int j = leftMostXAxis; j <= width; ++j)
            {
                if (grid_2[j, y] != null)
                {
                    grid_2[j, y - 1] = grid_2[j, y];
                    grid_2[j, y] = null;
                    grid_2[j, y - 1].transform.position -= new Vector3(0, 1, 0);
                }
            }
        }
    }
    ...
    bool ValidMove(GameObject piece)
    {
        foreach (Transform children in piece.transform)
        {
            ...
            if (grid_2[roundedX, roundedY] != null) return false;
        }
        return true;
    }
    ...
    public void RestartGame()
    {
        isGameOver = false;
        OnGameOver?.Invoke();
        if (PvPLineController.P2_roundWon == 2)
        {
    ...
    bool IsGameOver()
    {
        ...
        {
            if (grid_2[i, height] != null) return true;
        }
        return false;
    }
}
