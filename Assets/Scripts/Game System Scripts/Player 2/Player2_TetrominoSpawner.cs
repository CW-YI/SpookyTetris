...
public class Player2_TetrominoSpawner : TetrominoSpawner
{
    ...
    string[] tetrominoesNames = { "Player_2/Player2_I-Tetromino", "Player_2/Player2_J-Tetromino", "Player_2/Player2_L-Tetromino",
        "Player_2/Player2_O-Tetromino", "Player_2/Player2_S-Tetromino", "Player_2/Player2_T-Tetromino", "Player_2/Player2_Z-Tetromino" };
    ...
    void Update()
    {
        if (PhotonNetwork.IsConnected && !PhotonNetwork.IsMasterClient && startGame) HoldTetromino_Player2();
        else if (!PhotonNetwork.IsConnected && startGame) HoldTetromino_Player2();
    }
    ...
    public void NewTetromino()
    {
        if (nextTetrominoes.Count > 0)
        {
            ...
            currentTetromino.GetComponent<Player2_TetrisBlock>().enabled = true;
    ...
    public void InstantiateNextTetromino()
    {
        ...
        nextTetromino.GetComponent<Player2_TetrisBlock>().enabled = false;
    ...
    private void HoldTetromino_Player2()
    {
        if (Input.GetKeyDown(KeyCode.C) && !usedHold)
        {
            if (holdTetromino == null)
            {
                ...
                holdTetromino.GetComponent<Player2_TetrisBlock>().DestroyGhostPiece();
                holdTetromino.GetComponent<Player2_TetrisBlock>().enabled = false;
            ...
            {
                currentTetromino.GetComponent<Player2_TetrisBlock>().DestroyGhostPiece();

                currentTetromino.GetComponent<Player2_TetrisBlock>().enabled = false;
                currentTetromino.transform.rotation = Quaternion.identity;
                ...
                holdTetromino.GetComponent<Player2_TetrisBlock>().enabled = true;

                tempTetromino = currentTetromino;
                currentTetromino = holdTetromino;
                holdTetromino = tempTetromino;

                currentTetromino.GetComponent<Player2_TetrisBlock>().InstantiateGhostPiece();
                currentTetromino.transform.position = transform.position;
            }
            usedHold = true;
        }
